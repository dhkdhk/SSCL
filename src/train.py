...
def pretrain(encoder, dataloaders, args, device):
    mode = 'pretrain'
    ...
    for epoch in range(args.n_epochs):
        ...
        ''' epoch loop '''
        for i, (inputs, _) in enumerate(train_dataloader):             # 看一下dataloader 出来是两张增强图像
            ...
            #loss_simclr = encoder(x_i, x_j)
            loss = encoder(x_i, x_j,args,device)
            ...
            #loss = criterion_hcl(out_1, out_2, args.tau_plus,  args.beta, args.estimator, args, device)
            #loss = criterion(logit_simclr, label)
            #loss = criterion_hcl(out_1, out_2, 0.1, 1,  'hard', args)
            ...
            torch.cuda.synchronize()               # synchronize  just wait for all the work to be done on the GPU
            sample_count += inputs.size(0)         # sample_count  supervised train用到
            run_loss += loss.item()
        epoch_pretrain_loss = run_loss / len(dataloaders['pretrain'])

        ''' Update Schedulers '''
        # TODO: Improve / add lr_scheduler for warmup
        if args.warmup_epochs > 0 and epoch+1 <= args.warmup_epochs:                           # 更新学习率
        ...
        # else update patience.                                                                 # Number of Epochs to Wait for Improvement
        if epoch_pretrain_loss <= best_valid_loss:
            ...
            state = {
                ...
                'epoch': epoch,
                'loss': epoch_pretrain_loss,
            }
            torch.save(state, args.checkpoint_dir)                                             # 最好情况的参数存储
...
def supervised(encoder, dataloaders, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        epoch_pretrain_loss = run_loss / len(dataloaders['train'])  # loss 和 acc 都是算平均
...
def finetune(encoder, dataloaders, args):
    ''' Finetune script - MoCo
        ...
        encoder 只是网络backbone   并且只有fc   其他都固定
    '''
    ...
    for epoch in range(args.finetune_epochs):
        ...
        if valid_acc >= best_valid_acc:
            ...
            state = {
                ...
                'optimiser': optimiser.state_dict(),
                'epoch': epoch,
                'loss': epoch_finetune_loss,
                'acc': epoch_finetune_acc,
            }
            torch.save(state, (args.checkpoint_dir[:-3] + "_finetune.pth"))
        else:
            patience_counter += 1
            if patience_counter == (args.patience - 10):
                logging.info('\nPatience counter {}/{}.'.format(
